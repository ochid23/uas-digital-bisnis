        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y border-t">
                ...
                <tr class="hover:bg-slate-50/50 transition">
                    ...
                    <td class="px-8 py-6">
                        <img src="{{ ($event->poster_path && \Illuminate\Support\Facades\Storage::disk('public')->exists($event->poster_path)) ? asset('storage/' . $event->poster_path) : 'https://placehold.co/16x20' }}" class="w-16 h-20 rounded-xl object-cover shadow-sm">
                    </td>
                    <td class="px-8 py-6">
                        <p class="font-black text-slate-800">{{ $event->title }}</p>
                        <p class="text-xs text-slate-400">{{ $event->category->name ?? '-' }} • {{ \Carbon\Carbon::parse($event->date)->format('d-m-Y H:i') }}</p>
                    </td>
                    <td class="px-8 py-6">
