...
<body class="bg-zinc-950 text-white min-h-screen flex items-center justify-center p-6 antialiased">
    <div class="max-w-md w-full bg-zinc-900 text-zinc-100 rounded-[2.5rem] p-8 sm:p-10 shadow-2xl border border-zinc-800 my-8">
        ...
        <div class="text-center mb-8">
            ...
            <h1 class="text-2xl font-black text-white tracking-tight">Daftar Akun Baru</h1>
            <p class="text-zinc-400 text-sm mt-1">Bergabung dengan AmikomEventHub sekarang</p>
        </div>
        ...
        <form action="{{ route('register.post') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}" placeholder="Masukkan nama lengkap Anda" class="w-full px-5 py-4 bg-zinc-950 border border-zinc-800 rounded-2xl focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition font-medium text-white text-sm placeholder:text-zinc-600" required>
                @error('name')
            ...
            <div>
                ...
                <input type="password" name="password" placeholder="Minimal 6 karakter" class="w-full px-5 py-4 bg-zinc-950 border border-zinc-800 rounded-2xl focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition font-medium text-white text-sm placeholder:text-zinc-600" required>
                @error('password')
                    <p class="text-rose-400 text-xs mt-1.5 font-bold">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                <label class="block text-xs font-bold text-zinc-400 mb-2 uppercase tracking-wide">Konfirmasi Password</label>
                <input type="password" name="password_confirmation" placeholder="Ulangi password Anda" class="w-full px-5 py-4 bg-zinc-950 border border-zinc-800 rounded-2xl focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition font-medium text-white text-sm placeholder:text-zinc-600" required>
            </div>
        ...
        <div class="mt-6 flex items-center justify-between">
            ...
            <span class="text-[10px] text-center text-zinc-500 uppercase font-extrabold tracking-wider">Atau daftar dengan</span>
            <span class="border-b border-zinc-800 w-1/4"></span>
        </div>
        ...
        <div class="mt-8 pt-6 border-t border-zinc-800/80 text-center">
            <p class="text-xs text-zinc-400 font-medium">
                Sudah memiliki akun?
                <a href="{{ route('login') }}" class="text-indigo-400 hover:text-indigo-300 font-black transition ml-1 underline decoration-indigo-500/30">
                    Masuk di sini
                </a>
            </p>
        </div>
