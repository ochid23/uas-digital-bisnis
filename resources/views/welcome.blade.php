...
@section('content')
    ...
    <section id="events" class="max-w-7xl mx-auto px-6 py-20">
        ...
        <div class="mb-10 flex gap-4 flex-wrap justify-start">
            <a href="{{ url('/') }}#events" 
               class="px-6 py-2.5 rounded-xl font-bold transition {{ !request('category') ? 'bg-indigo-600 text-white shadow-lg' : 'bg-white border border-slate-200 text-slate-600 hover:border-indigo-600' }}">
                Semua Kategori
            </a>
            
            @foreach($categories as $cat)
                <a href="{{ url('/?category=' . $cat->slug) }}#events" 
                   class="px-6 py-2.5 rounded-xl font-bold transition {{ request('category') == $cat->slug ? 'bg-indigo-600 text-white shadow-lg' : 'bg-white border border-slate-200 text-slate-600 hover:border-indigo-600' }}">
                    {{ $cat->name }}
                </a>
            @endforeach
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div class="col-span-full text-center py-20">
                <div class="bg-slate-50 rounded-3xl p-12 border-2 border-dashed border-slate-200">
                    <p class="text-slate-500 font-medium text-lg">Belum ada event tersedia untuk kategori ini.</p>
                </div>
            </div>
            @endforelse
        </div>
    ...
    <section class="max-w-7xl mx-auto px-6 py-16 border-t border-slate-100">
        <div class="text-center mb-10">
            <h2 class="text-2xl font-bold text-slate-700">Telah Didukung Oleh Platform AmikomEventHub</h2>
        </div>
